...
def simulate(params = [0.5,3,0.01,0.01], dimension1 = 'lines',subject = '0', rng = None, seed = 42):
    ...
    'alpha' - feature learning rate parameter
    'beta' - choice determinism parameter
    'theta0' - dimension primacy parameter
    'epsilon' - dimension learning rate parameter
        ...
        for st in range(maxTrials): # loop over number of trials in each stage
            ...
            if (st >= 5)  & (all(RFeature[stage] in x for x in choice[-6:])):
                ...
                results = {'PE':PE, 'choice': choice,'R':R, 'r':r, 'S':S, 'inputs':inputs, 'WH1':WH1,\
                           'WH2':WH2, 'THE':THE,'alpha':alpha,'beta':beta,'theta0':theta0,'epsilon':eps,'wh0':wh0,\
