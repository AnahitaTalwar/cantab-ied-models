...
def simulate(params = [0.5,3], dimension1 = 'lines',subject = '0', rng = None, seed = 42):
   ...
   """
    ...
    inputs,AH,V,choice,R,WH1,WH2,PE,S,r = [],[],[],[],[],[wh1.copy()],[wh2.copy()],[np.array([0.])],[],[]
    ...
    for stage in range(len(stages)): # loop over number of stages
        for st in range(maxTrials): # loop over number of trials in each stage
            ...
            if (st >= 5)  & (all(RFeature[stage] in x for x in choice[-6:])):
                ...
                results = {'PE':PE, 'choice': choice,'R':R, 'r':r, 'S':S, 'inputs':inputs, 'WH1':WH1,\
                           'WH2':WH2, 'alpha':alpha,'beta':beta, 'wh0':wh0,\
                            'dimension1':dimension1,'subject':subject,'stage':list(itertools.chain(*[[stages[x]]*Trials[x] for x in range(len(Trials))])),\
                            'Trials':Trials,'Errors':Errors,'Stages':Stages,'Passed':Passed}
